		...
		this.showPicker = function(callback,engine,sideLength_){
			...
			if(engine == "L" ){
				...
				$("#bigSections li").hover(function(){
					var $this = $(this);
					// console.log($this.position().left+2, $this.position().top+2);
				...
				$("#bigSections").unbind("mousemove").unbind("mouseout").mousemove(function(event){
					...
					// console.log($this.position().left + xi*11, $this.position().top + yi*11);
					$("#bigLayout").css({"left":$this.position().left + xi*11+1,"top":$this.position().top + yi*11+2}).show();
...
//				console.log(2);
				$(currentPicker).css({ 'background-color': hex });
				$(currentPicker).attr('value',hex);
				console.log(hex);
			}else{
				console.log(3);
