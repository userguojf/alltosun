...
class Action
{
    public function test6(){
        $list = _model('probe_device')->getList([1=>1]);

        foreach ($list as $v) {
            //probe_device_mac_res
            
            $mac = _uri('probe_device_mac_res', ['device' => $v['device']], 'mac');

            if ($mac) {
                _model('probe_device')->update($v['id'], ['mac' => $mac]);
            }
        }
    }
    ...
    public function test7(){
        ...
        foreach ($list as $v) {
           if (filter_var(trim($v['device']), FILTER_VALIDATE_MAC)) {
               _model('probe_device')->update($v['id'], ['mac' => $v['device']]);
           } 
        }
    }
    ...
    {
        $data = file_get_contents('http://201711awifiprobe.alltosun.net/images/data/glt_mac_data');

        $data = explode(chr(10) ,  $data);

        foreach ($data as $v) {
            $info = explode(chr(9), $v);

            $r = _model('probe_device_mac_res')->create([ 'mac' => strtolower(trim($info[0])), 'device' => trim($info[1]) ]);
