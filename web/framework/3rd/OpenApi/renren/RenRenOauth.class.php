...
class RenRenOauth extends RESTClient {
	...
	public function getAuthorizeUrl() {
	    // add scope by guoyx 2011-12-29
		$url = $this->_config->AUTHORIZEURL . '?response_type=code&client_id=' . $this->_config->APIKey . '&redirect_uri=' . urlencode($this->_config->CALLBACK).'&scope='.$this->_config->SCOPE;
	...
	public function getSessionKey($access_token) {
		...
		$ret = $this->call($url, 'POST', $params);
		$ret = json_decode($ret, true);
		mail('liw@alltosun.com', 'renren_ret', var_export($ret, true));
		# check error
		if(isset($ret['error_description'])) {
			throw new Exception($ret['error_description']);
		}
