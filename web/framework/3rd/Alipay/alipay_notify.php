	...
	*return 验证结果：true/false
    ...
    function return_verify() {
		...
		if(empty($_GET)) {							//判断GET来的数组是否为空
		    ...
		    // @FIXME
		    unset($_GET['url']);
                   // 框架路由使用anu导致签名失败的问题 liw modified 2013-10-11
                    unset($_GET['anu']);
//		    foreach ($_GET as $k=>$v) {
//		        $_GET[$k] = urldecode($v);
//		    }
//		    var_dump($_GET);
			...
			if (preg_match("/true$/i",$veryfy_result) && $this->mysign == $_GET["sign"]) {
			    ...
			    // test veryfy_result
//			    echo 'veryfy_result>>>>>>>';
//			    var_dump($veryfy_result, $this->mysign, $_GET["sign"]);
//			    echo '<<<<<<<<<<<$_GET[sign]>>>>>>>>>>';
//			    $veryfy_url = urldecode($veryfy_url);
//			    $veryfy_result = $this->get_verify($veryfy_url);
//			    var_dump($veryfy_result);
//			    echo '<<<<<<<<<<<urldecode once end>>>>>>>>>';
//			    $veryfy_url = urldecode($veryfy_url);
//			    $veryfy_result = $this->get_verify($veryfy_url);
//			    echo '<<<<<<<<<<<urldecode twice end>>>>>>>>>';
				return false;
			}
		}
    }
