...
///////////////////////////wangjf add: 记录log//////////////////////////////
        ...
        if (!is_null($body)) {
            if ($body) {
                self::write_log($url, $httpCode, json_encode($request_body), $body, 'push_http', '0');
            } else {
                self::write_log($url, $httpCode, json_encode($request_body), $output, 'push_http', '0');
            }
        }
///////////////////////////////////////////////////////////////////////////////////
    ...
    /**
     * 写入错误日志
     * @param unknown $request 请求数据
     * @param unknown $response 返回数据
     * @param unknown $res_name res_name  此处是 jpush_xxx
     * @param unknown $device_unique_id 设备唯一标识
     */
    public static function write_log($url, $code, $request, $response, $res_name, $device_unique_id)
    {
        $new_log = array(
                'api_path'      => $url,
                'res_name'      => $res_name,
        ...
        _model('push_log')->create($new_log);
    }
}
