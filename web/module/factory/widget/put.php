 <?php
 /**
  * alltosun.com 亮屏内容发布widget put.php
 * ============================================================================
 ...
 */
 class put_widget
 {
      ...
      * 发布内容
      */
     public function put_content($param=array())
     {
         //初始化（接收参数）
         $this->put_content_init($param);
         ...
         if (is_array($result)) {
             ...
             foreach ($result as $v) {
                 $registration_id = $this->get_put_registration_ids(_uri('screen_content_res', $v));

                 if ($registration_id === false) {
                     continue;
                 }

                 if ($registration_id && is_array($registration_id)) {
             ...
             $registration_ids = $this->get_put_registration_ids(_uri('screen_content_res', $result));
         }
      ...
      * 发布内容初始化
      */
     private function put_content_init($param=array())
     {
             $this->member_id         = member_helper::get_member_id();
         if (empty($param)) {
             $this->business_hall_ids = tools_helper::post('business_hall_ids', array());
     ...
     private function put_content_check()
     {
         ...
         $content_info = _model('screen_content')->read($this->content_id);
         ...
         //验证专属机型图是否指定机型
         if ( $content_info['is_specify'] && $content_info['type'] == 4 && (!$this->phone_name || !$this->phone_version || $this->phone_name == 'all' || $this->phone_version == 'all')) {
     ...
     private function business_hall_put($filter)
     {
         ...
         if ($count > 1) {
             //先删除存在的厅发布信息
             $exits_ids = _model('screen_content_res')->getFields('id', array(
             ...
             if ($exits_ids) {
                 _model('screen_content_res')->delete($exits_ids);
             }
         }
         ...
         foreach ($this->business_hall_ids as $v) {
             if ($v) {
                 ...
                 if (_uri('screen_content_res', $filter)) {
                 ...
                 $new_res_id[] = _model('screen_content_res')->create($filter);
             }
         }
     ...
     private function area_put($filter)
     {
         ...
         $exits_ids = _model('screen_content_res')->getFields('id', $delete_filter);
         ...
         return _model('screen_content_res')->create($filter);
     }
      ...
      * @param int $res_id
      */
     public function is_content_put($content_id , $res_name, $res_id = 0, $device_info, $table='screen_content_res')
     {
