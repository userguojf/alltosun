<?php
/**
  * alltosun.com 山脊设备数据处理 shanj.php
  * ============================================================================
  ...
  * $Date: 2017年6月15日 下午5:38:14 $
  * $Id$
  */
class shanj extends BaseHandle
{
    //数据字段
    ...
    public function string_to_arr($string, $fd=NULL)
    {
        ...
        if (isset($original_arr['MessageId']) && $original_arr['MessageId'] != '--') {
            if (is_object(self::$serv)) {
                //发送应答
                self::$serv->send($fd, "MessageId:".$original_arr['MessageId']."\n");
            }
        } else {
            return array('errno' => 1001, 'msg' => '数据缺少MessageId');
        }

        //处理ID
        if ( isset($original_arr['MAC']) && $original_arr['MAC'] && $original_arr['MAC'] != '--') {
            $new_arr['ID'] = $original_arr['MAC'];
        } else {
            return array('errno' => 1001, 'msg' => '数据缺少MAC');
        }

        //山脊的设备暂时只处理放下包，因放下的包中拿起和放下的时间都存在
        if ($original_arr['MOV'] != '00') {
            return array('errno' => 1002, 'msg' => "数据包无需处理");
        }

        //处理请求时间
        if ( isset($original_arr['START']) && $original_arr['START'] != '--' && isset($original_arr['END']) && $original_arr['END'] != '--') {
            $new_arr['start_timestamp']     = floor($original_arr['START']/1000);
            $new_arr['end_timestamp']       = floor($original_arr['END']/1000);
        } else {
