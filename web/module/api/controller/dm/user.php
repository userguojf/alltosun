...
//             2 => 'update',
    ...
    */
    public function __construct()
    {
        ...
        // 时间戳过期(time、 数字  、即使是服务器时间不一致 半个小时足够了)
//         if (  ) {
...
//         }
    ...
    private function comprehensive($result, $log_info, $errmsg)
    {
        ...
        if (is_array($errmsg) && isset($errmsg['errcode'])) {
            ...
            // 重新赋值为字符串
            $errmsg = _widget('wework.errmsg')->get_errmsg($errmsg['errcode']);
        }
    ...
    */
    ...
    private function check_qydev_user_info($user_number, $phone)
    {
        ...
        if ($local_user_info && $local_user_info['an_id'] != $user_number) {
            $log_info = array('response' => '该手机号与其他营业厅渠道码已绑定');
            $this->comprehensive(false, $log_info, '该手机号与其他营业厅渠道码已绑定');
        }

        //此渠道、手机号是否同时存在
        $special_user_info = _model('wework_user')->read(array('an_id' => $user_number, 'mobile' => $phone));

        if ($special_user_info) {
            //获取企业号的成员信息（主要需要的是部门ID，其他信息不能随意更改，因为可能会影响其他的业务线）
            $qydev_user_info = _widget('wework.user')->get_user_info($special_user_info['user_id']);
...
//         }
    ...
    private function handle_department_info($business_hall_title, $province, $project_depart_id_arr)
    {
        ...
        foreach ($project_depart_id_arr as $v) {
            ...
            if ( in_array($v, $this->special_depart_id) ) {
                array_push($ids, $v);
                continue;
            }
            ...
            //第二步 查看省三级部门信息
            $local_pro_depart = _model('wework_department')->read(array('pid'=>$v , 'name'=>$province));
            ...
            if (!$local_pro_depart) {
                ...
                //第三步  企业号创建   省级部门
                $pro_depart_info = _widget('wework.department')->create_department($pro_params);
                ...
                array_push($ids, $local_pro_depart['depart_id']);
            }
        }
    ...
    private function handle_user_info($name, $phone, $weixin_id, $business_hall_title, $user_number, $yyt_depart_id_arr)
    {
        ...
        $params['name']  = $name;
        $params['mobile'] = $phone;
        $params['analog_i']  = $user_number;
        $params['an_id']     = $user_number;
        $params['business_hall'] = $business_hall_title;

        //查看本地该渠道码 最后一条
        $local_user_info = _model('wework_user')->read(array('user_id LIKE' => "%".$user_number."%"),  " ORDER BY `user_id` DESC ");
    ...
    private function qydev_user_operation($qydev_user_info, $yyt_id)
    {
       ...
       //拼接数据（循环拼接部门ID）
        ...
        _model('wework_user')->update(
            array('user_id' => $qydev_user_info['userid']),
    ...
    {
        _model('wework_department')->create(
            array(
                'depart_id' => $depart_id,
                'name'      => $name,
                'pid'       => $parent_id
