...
class Action
{
    ...
    public function add()
    {
        ...
        $this->return_data(0, 'success', array('shoppe_id' => $result));
    ...
    public function delete()
    {
        ...
        //是否存在亮屏设备
        $device_info = _model('screen_device')->read(array('shoppe_id' => $shoppe_id, 'status' => 1));
        ...
        //是否存在RFID设备
        $device_info = _model('rfid_label')->read(array('shoppe_id' => $shoppe_id));

        if ($device_info) {
            $this->return_data(1, '专柜无法删除：此专柜存在RFID设备');
        }
    ...
    public function generate_postfix()
    {
        ...
        if (!$postfix) {
            $this->return_data(1, "[{$shoppe_name}]系列在本厅已超出最大限制[十]", array());
        };
