...
class AnForm
{
    ...
    public static function generateHtml($params)
    {
        ...
        if ($params['type'] || $params['action']) {
            if (isset($params['type']) && !in_array($params['type'], array('text', 'password', 'select', 'checkbox', 'radio', 'textarea', 'hidden', 'submit', 'reset', 'file' ,'number'))) {
...
//         }
    ...
    /**
     * 生成input格式的html 包括input/password/hidden/submit/reset/Number
     * @param array $params
     * @return string
     * @author add  shenxn
     */
    ...
    private static function generateSelect($params)
    {
        ...
        $select .= ' >';
        $select .= '<option value='.$params['default_value'].'>'.$params['default_text'].'</option>';
    ...
    private static function generateCheckbox($params)
    {
        ...
        if ($params['checkboxes'] && is_array($params['checkboxes'])) {
            foreach ($params['checkboxes'] as $k=>$v) {
                ...
                $checkboxes .= ' /><span for="'.$label_id.$k.'">'.$v.'</span>&nbsp;&nbsp;';
                $i++;
    ...
    private static function generateRadio($params)
    {
        ...
        if ($params['radios'] && is_array($params['radios'])) {
            foreach ($params['radios'] as $k=>$v) {
                ...
                $radio .= ' /><span for="'.$label_id.$k.'">&nbsp;&nbsp;'.$v.'&nbsp;&nbsp;</span>';
                $i++;
    ...
    /*submitHandler: function(form) {
                                $("input[type=submit]", $(form)).attr("disabled", "disabled").val("提交中");
                               	form.submit();
                              }*/
    /**
     * 验证数据是否必填
