...
//                 _model('screen_version')->update($new_info['id'], array('down_num'=>$new_info['down_num']+1));
    ...
    public function update_version_info()
    {
        ...
        $mode  = tools_helper::post('mode', 0);
        $code  = tools_helper::post('code', '');
        $info  = tools_helper::post('info', '');
        $handlePatchVersion  = tools_helper::post('handlePatchVersion', '');
        ...
        $new_data = array(
                ...
                'handle_patch_version'    => $handlePatchVersion,
                'info'                  => $info,
                'code'                  => $code,
                'mode'                  => $mode
        );

        _model('screen_update_version_info')->create($new_data);
