...
class Action
{
    public function add()
    {
        ...
        if (!$user_number) {
            api_helper::return_api_data(1004, '请输入营业厅编码', array(), $api_log_id);
        }

        if (!$device_unique_id) {
            api_helper::return_api_data(1004, '请输入设备唯一标识', array(), $api_log_id);
        }
        ...
        if (!$business_info) {
            api_helper::return_api_data(1004, '营业厅不存在',  array(), $api_log_id);
        }

        if ( !$device_info ) {
            //没有设备则查询是否自动下架
            if (!screen_device_helper::is_auto_dropoff($device_unique_id)) {
                api_helper::return_api_data(2003, '设备不存在或已被手动下架', array(), $api_log_id);
            }
            ...
            //已自动下架，设置为上架
            _model('screen_device')->update($update_filter, array('status' => 1));

            $device_info = screen_device_helper::get_device_info_by_device($device_unique_id);

            //自动上架记录
            screen_device_helper::drop_off($device_info, 102);
        }
...
//         }
        ...
        screen_helper::add_device_online_stat_day($stat_info, 'v4');
