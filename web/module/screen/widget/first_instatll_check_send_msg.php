...
// exit('暂时中止');
        ...
        $sended_info = _model('screen_qydev_msg_record')->getFields( 'business_hall_id',
                array(
                        'res_name' => 'check',
                        'type'     => 0,
                )
        ...
        foreach ($list as $k => $v) {
            // 过滤发过短信的营业厅
            if ( in_array($v['business_hall_id'], $sended_info) ) {
                continue;
            }
    ...
    public function by_user_number_qydev_user_id($user_number, $device_offline_num, $business_hall_id)
    {
        ...
        // 发消息  最多支持1000个
        $this->send_msg(trim($touser, '|'), $device_offline_num, $business_hall_id);
        // 发短信
