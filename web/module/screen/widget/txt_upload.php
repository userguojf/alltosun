<?php
/**
  * alltosun.com txt文件widget txt_upload.php
  * ============================================================================
  ...
  * $Id$
  */
class txt_upload_widget
{
    //设置文件大小不超过5MB
    public static $max_size_file_upload =  '5000000';

    //允许上传的文件扩展名
    public $array_extention_interdite = array('.txt');

    //允许上传的资源名
    public $array_res_name = array(
            'add_device_record' => 1,       //动作
            'content_stat'      => 1,       //内容统计（轮播）
            'add_click'         => 1,       //点击统计
    );
    ...
    {
        self::$upload_dir = UPLOAD_PATH.'/act_data';
    }

    public function upload_txt($name, $res_name)
    {
        ...
        if (empty($this->array_res_name[$res_name])) {
            return array('errno' => 1001,  'msg' => '未许可的资源不可上传');
        }

        $txt_name = $this->build_txt_name('txt', $res_name);

        if (move_uploaded_file( $file_info['tmp_name'], $txt_name)){
            /*
             * 视频压缩  暂时不部署到线上
             *   //获取后缀
             ...
             */
            return array('errno' => 0,  'file' => str_replace(self::$upload_dir, '', $txt_name));
        }
     ...
     */
    public function build_txt_name($ext, $res_name)
    {
        $time = time();
        $folder = self::$upload_dir.'/'.$res_name.date('/Y/m/d', $time);
        ...
        if (file_exists($target)) {
            return $this->build_txt_name($ext);
        }
     ...
     */
    public function _txt($path, $res_name)
    {
        if (!$path) return '';
    ...
    {
        if (!$crf) {
            $crf = screen_content_config::$ffmpeg_crf;
        }
