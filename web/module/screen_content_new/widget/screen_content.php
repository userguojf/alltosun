...
class screen_content_widget
{
    ...
    public function get_device_roll_content($device_info)
    {
        ...
        //根据归属地或者机型分组
        foreach (screen_content_new_config::$content_issuer_res_name_type as $region_k => $region_v) {
            //当前归属的内容
            $region_curr = array();
            //循环内容
    ...
    public function perfection_region_content($region_content_list)
    {
        ...
        foreach ($region_content_list as $k => $v) {
            ...
            if ($v['type'] == 3) {
                ...
                if ($v['roll_interval'] < 1 && !screen_content_new_helper::is_animated_gif(UPLOAD_PATH.'/'.$v['link'])) {
                    $v['roll_interval'] = 10;
                }
    ...
    public function perfection_set_meal_content($set_meal, $device_info)
    {
        ...
        foreach ( $set_meal as $k => $v ) {
            ...
            //获取套餐图
            $set_meal_info = screen_content_new_helper::get_set_meal($device_info, $v);
    ...
    public function make_thumb($file_path, $prefix = 'middle') {
        ...
        //是否为动图, 动图不处理
        $is_animated_gif = screen_content_new_helper::is_animated_gif($file_path);
        if ($is_animated_gif) {
            return false;
        }
