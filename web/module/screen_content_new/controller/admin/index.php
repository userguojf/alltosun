...
class Action
{
    ...
    public function __call($action = '', $params = array())
    {
        ...
        if (empty($search_filter['put_type']) || $search_filter['put_type'] == 1) {
            ...
            if ($this->member_res_name == 'group') {
                ...
                if (!empty($search_filter['creator']) && isset($search_filter['creator'])) {
                    $filter['res_name'] = $search_filter['creator'];
                }
            } else {
                //查询所有厅id
                ...
                if ($this->member_res_name == 'province') {
                    ...
                    $group = _model('screen_content')->getFields('id', array('res_name' => 'group', 'res_id' => 0));
                    $city = _model('screen_content')->getFields('id', array('res_name' => 'city', 'res_id' => $city_ids));
                    $area = _model('screen_content')->getFields('id', array('res_name' => 'area', 'res_id' => $area_ids));
                    $business_hall = _model('screen_content')->getFields('id', array('res_name' => 'business_hall', 'res_id' => $business_ids));

                    if (!empty($search_filter['creator']) && isset($search_filter['creator'])) {
                        $creator = $search_filter['creator'];
                        $content_ids = $$creator;
                    } else {
                        $content_ids = array_merge($group ,$city, $area, $business_hall);
                    }
                    ...
                    $group = _model('screen_content')->getFields('id', array('res_name' => 'group', 'res_id' => 0));
                    $province = _model('screen_content')->getFields('id', array('res_name' => 'province', 'res_id' => $province_id));
                    $area = _model('screen_content')->getFields('id', array('res_name' => 'area', 'res_id' => $area_ids));
                    $business_hall = _model('screen_content')->getFields('id', array('res_name' => 'business_hall', 'res_id' => $business_ids));
                    if (!empty($search_filter['creator']) && isset($search_filter['creator'])) {
                        $creator = $search_filter['creator'];
                        ...
                        $content_ids = array_merge($group, $province, $area, $business_hall);
                    }
                    ...
                    $group = _model('screen_content')->getFields('id', array('res_name' => 'group', 'res_id' => 0));
                    $province = _model('screen_content')->getFields('id', array('res_name' => 'province', 'res_id' => $province_id));
                    $city = _model('screen_content')->getFields('id', array('res_name' => 'city', 'res_id' => $city_id));
                    $business_hall = _model('screen_content')->getFields('id', array('res_name' => 'business_hall', 'res_id' => $business_ids));
                    ...
                    if (!empty($search_filter['creator']) && isset($search_filter['creator'])) {
                        ...
                        $content_ids = array_merge($group, $province, $city, $business_hall);
                    }
                    ...
                    $province = _model('screen_content')->getFields('id', array('res_name' => 'province', 'res_id' => $province_id));
                    $city = _model('screen_content')->getFields('id', array('res_name' => 'city', 'res_id' => $city_id));
                    $area = _model('screen_content')->getFields('id', array('res_name' => 'area', 'res_id' => $area_id));
                    if (!empty($search_filter['creator']) && isset($search_filter['creator'])) {
                        $creator = $search_filter['creator'];
                        $content_ids = $$creator;
                    } else {
                        $content_ids = array_merge($group, $province, $city, $area);
                    }
                }
        ...
        // 筛选类型
        if (!empty($search_filter['content_type'])) {
            $filter['type'] = $search_filter['content_type'];
        }
        ...
        //added by guojf
        $make_pic_filter = array(
                    'res_id'     => $this->member_info['res_id'],
                    'res_name'   => $this->member_info['res_name'],
                    'status'     => 1
        );

        $pic_list = _model('screen_content_make_pic_record')->getList(
                $make_pic_filter,
                "ORDER BY `id` DESC LIMIT 7 "
        );

        $make_pic_filter['use_num >'] = 0;

        $hot_pic_info = _model('screen_content_make_pic_record')->read(
                $make_pic_filter,
                "ORDER BY `use_num` DESC LIMIT 1 "
        );

        $new_pic_info   = [];

        if ( $pic_list ) {
            $shift_pic_list = $pic_list;

            for ($i = 1 ; $i < 4; $i ++) {
                if ( !$shift_pic_list ) break;

                $new_pic_arr = array_shift($shift_pic_list);

                if ( $hot_pic_info ) {
                    if ( $new_pic_arr == $hot_pic_info ) {
                        continue;
                    } else {
                        $new_pic_info[] = $new_pic_arr;
                    }
                    if ( 2 == count($new_pic_info )) break;
                } else {
                    $new_pic_info[] = $new_pic_arr;
                }
            }
        ...
        Response::assign('pic_list', $pic_list);
        Response::assign('hot_pic_info', $hot_pic_info);
        Response::assign('new_pic_info', $new_pic_info);
    ...
    {
        // added by guojf
        $content_info['link'] = tools_helper::get('src', '');
        $model_id = tools_helper::get('model_id', 0);
        $this->select_data();
        $field = 'name';
        ...
        // 当前用户登录的地区
        $name = _uri($this->member_res_name, $this->member_res_id, $field);
        if ($this->member_res_name == 'group') {
            $name = '全国';
        }

        // added by guojf
        if ($content_info['link']) {
            $content_info['type'] = 1;
            Response::assign('content_info', $content_info);
        }

        Response::assign('name', $name);
        Response::assign('model_id', $model_id);
    ...
    public function set_meal_add()
    {
        ...
        if ($content_id) {
            ...
            if (!$content_info || $content_info['status'] == 2) {
                ...
                $set_meal_list = _model('screen_content_set_meal')->getList(array('content_id' => $content_info['id']));
    ...
    public function edit()
    {
        ...
        if ($content_info['type'] == 3) {
            ...
            //是否为动图
            if (screen_content_new_helper::is_animated_gif($link_path)){
                $type = 2;
            } else {
                $type = 1;
            }
        ...
        //投放列表
        $content_list = _model('screen_content_res')->getList(array('content_id' => $id));
    ...
    public function save()
    {
        ...
        $put_type    = Request::Post('put_type', 0);
        ...
        if ($content_id) {
            ...
            if ($model_id) {
                ...
                screen_content_new_helper::use_num_record($model_id);
            }
        ...
        if ($put_type == 1) {
            ...
            $list = _model('screen_content_res')->getList(['content_id'=>$content_id]);
            foreach ($list as $v) {
                //推送
                $this->push_by_content_res(_uri('screen_content_res', $v['id']), '2');
            }
            if ($res != 'ok') {
                return '发布失败!';
            }
        ...
        header('location:'.AnUrl("screen_content_new/admin"));
    }
    ...
    public function region_range_put($content_id, $type=0 )
    {
        ...
        if ($type == 5) {
            ...
            foreach ( $set_meal_list as $k => $set_meal_info ) {
                ...
                _widget('screen_content_new.put')->put_content($param);
            }

            return true;

        } else {
            return _widget('screen_content_new.put')->put_content($param);
        }
...
//     }
    ...
    public function put() {
        ...
        $content_info = screen_content_new_helper::get_content_info($content_id);
...
//         ->setCellValueExplicit('A28', '请尽量保持在五个中文汉字的宽度之内');
    ...
    // js load加载局部内容
    public function put_content_list()
    {
        $id= tools_helper::Get('content_id', 0);
        $content_info = _uri('screen_content', $id);
        //投放列表
        $content_list = _model('screen_content_res')->getList(array('content_id' => $id));
        foreach ($content_list as $k => $v) {
        ...
        Response::assign('content_list', $content_list);
        Response::assign('content_info', $content_info);
        Response::display('admin/put_content_list.html');
    }
