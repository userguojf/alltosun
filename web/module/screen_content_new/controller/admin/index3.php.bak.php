...
class Action
{
    ...
    public function region_range_put($content_id, $type=0 )
    {
        ...
        //卖点套餐图, 查出机型 wangjf add 2018-05-29
        if ($type == 5) {
            //查询套餐详情
            $set_meal_info = _model('screen_content_set_meal')->read(array('content_id' => $content_id));
            ...
            //查询品牌型号
            $nickname_info = screen_device_helper::get_device_nickname_info(array('name_nickname' => $set_meal_info['phone_name'], 'version_nickname' => $set_meal_info['phone_version']));
     ...
     * 表单的方式
     */
    public function save_set_meal($content_id)
    {
        $set_meal_data = tools_helper::Post('set_meal', array());
