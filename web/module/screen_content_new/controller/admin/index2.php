...
class Action
{
    ...
    public function __call($action = '', $params = array())
    {
        ...
        Response::display("admin/content_list2.html");
    }
    ...
    public function add()
    {
        ...
        Response::display("admin/add2.html");
    }
    ...
    public function save()
    {
        ...
        if ($content['type'] == 1 || $content['type'] == 4) {
            ...
            if (!$content_id) {
                ...
                if($str){
                    Response::assign('error_list', $data);
                    Response::display("admin/add2.html");
                    return;
                }
                if (!is_array($data)) {
        ...
        if ($put_type == 1) {
            ...
            //范围投放
            $res = $this->region_range_put($content_id);
    ...
    public function load_set_meal_template()
    {
        ...
        ->setCellValue('D1', '（必填）合约价')
        ->setCellValue('E1', '（必填）推荐档位')
        ->setCellValue('F1', "（必填）卖点1（在五个汉字的宽度）")
        ->setCellValue('G1', "（必填）卖点2（在五个汉字的宽度）")
        ->setCellValue('H1', "（必填）卖点3（在五个汉字的宽度）")
        ->setCellValue('I1', "（必填）设备参数1（在五个汉字的宽度）")
        ->setCellValue('J1', "（必填）设备参数2（在五个汉字的宽度）")
        ->setCellValue('K1', "（必填）设备参数3（在五个汉字的宽度）")
        ->setCellValue('L1', "（必填）设备参数4（在五个汉字的宽度）")
        ->setCellValue('M1', "（必填）设备参数5（在五个汉字的宽度）")
        ->setCellValue('N1', "（必填）设备参数6（在五个汉字的宽度）");
...
//         ->setCellValueExplicit('A28', '请尽量保持在五个中文汉字的宽度之内');
    ...
    public function down_error_list()
    {
        ...
        ->setCellValue('D1', '（必填）合约价')
        ->setCellValue('E1', '（必填）推荐档位')
        ->setCellValue('F1', "（必填）卖点1（五个汉字的宽度）")
        ->setCellValue('G1', "（必填）卖点2（五个汉字的宽度）")
        ->setCellValue('H1', "（必填）卖点3（五个汉字的宽度）")
        ->setCellValue('I1', "（必填）设备参数1（五个汉字的宽度）")
        ->setCellValue('J1', "（必填）设备参数2（五个汉字的宽度）")
        ->setCellValue('K1', "（必填）设备参数3（五个汉字的宽度）")
        ->setCellValue('L1', "（必填）设备参数4（五个汉字的宽度）")
        ->setCellValue('M1', "（必填）设备参数5（五个汉字的宽度）")
        ->setCellValue('N1', "（必填）设备参数6（五个汉字的宽度）");
...
//         $objPHPExcel->getActiveSheet()->getCell( 'A18')->setValue($objRichText);
        ...
        foreach ($error_list as $k => $v) {
            ...
            ->setCellValueExplicit('D'.$i, $v['contract_price'], PHPExcel_Cell_DataType::TYPE_STRING)
            ->setCellValueExplicit('E'.$i, $v['recommended_position'], PHPExcel_Cell_DataType::TYPE_STRING)
            ->setCellValueExplicit('F'.$i, $v['selling_point_1'])
            ->setCellValueExplicit('G'.$i, $v['selling_point_3'])
            ->setCellValueExplicit('H'.$i, $v['selling_point_5'])
            ->setCellValueExplicit('I'.$i, $v['param_1'])
            ->setCellValueExplicit('J'.$i, $v['param_2'])
            ->setCellValueExplicit('K'.$i, $v['param_3'])
            ->setCellValueExplicit('L'.$i, $v['param_4'])
            ->setCellValueExplicit('M'.$i, $v['param_5'])
            ->setCellValueExplicit('N'.$i, $v['param_6']);
            ...
            if(strlen($v['selling_point_1'])>15){
                $objRichText = new PHPExcel_RichText();
                $objPayable = $objRichText->createTextRun('超过宽度');
                $objPayable->getFont()->setColor( new PHPExcel_Style_Color( PHPExcel_Style_Color::COLOR_RED ) );
                $objPHPExcel->getActiveSheet()->getCell( 'F'.$i)->setValue($objRichText);
            }
            ...
            if(strlen($v['param_5'])>15){
                $objRichText = new PHPExcel_RichText();
                $objPayable13 = $objRichText->createTextRun('超过宽度');
            ...
            if(!$v['param_6']){
                ...
                $objPayable = $objRichText->createTextRun('必填');
                $objPayable->getFont()->setColor( new PHPExcel_Style_Color( PHPExcel_Style_Color::COLOR_RED ) );
                $objPHPExcel->getActiveSheet()->getCell( 'N'.$i)->setValue($objRichText);
            }
    ...
    public function check_meal($str,$msg,$i)
    {
