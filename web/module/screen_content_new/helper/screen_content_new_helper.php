...
/**
  ...
  * $Id$
  */
class screen_content_new_helper
{
    /**
     * 获取内容详情
...
//            }
            ...
            if (strpos($content_info['link'],'http') === false) {
                $content_info['link'] = 'http://'.$content_info['link'];
            }
            return '<a href="'.$content_info['link'].'" target="_blank">点击跳转</a>';
        }
    }
    ...
    public static function use_num_record($model_id)
    {
        if ( !$model_id ) return false;

        $pic_info = _model('screen_content_make_pic_record')->read(array('id' => $model_id));

        if ( !$pic_info ) {
            return false;
        }

        _model('screen_content_make_pic_record')->update(
            array('id' => $model_id),
    ...
    public static function get_use_num($model_id)
    {
        if ( !$model_id ) return false;

        $content_ids = _model('srceen_content')->getFields('id', array('model_id' => $model_id));
