...
class Action
{
    ...
    public function stat()
    {
        ...
        $list = _model('screen_daily_hebave_device_record')->getList(
                array('id >' => $id, 'status' => 0, 'record_day <' => 20180730 ), " LIMIT {$limit} " );
        ...
        echo "<script>window.location.href = '". AnUrl("screen_stat/admin/run_data/stat?id={$this->id}")."'</script>";
    }
    ...
    private function handle_device_info($device_info_list)
    {
        ...
        $param['boot_time'] = strtotime(date('Y-m-d 08:00:00', strtotime($this->filter['record_day'])));
        $param['down_time'] = strtotime(date('Y-m-d 20:00:00', strtotime($this->filter['record_day'])));
    ...
    private function create_happpenind_record($filter)
    {
        ...
        if (  $filter['boot_time'] > $filter['down_time'] ) {
            $filter['down_time'] = strtotime(date('Y-m-d 20:00:00', strtotime($this->filter['record_day'])));
        }
