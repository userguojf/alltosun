...
//             $filter['status >'] = 1;
    ...
    public function detail_apply()
    {
        ...
        if($this->res_name == 'group'){
            //符合条件的所有
            $apply_info = _model('apply')->getList($filter,$order);
           foreach ($apply_info as $k => $v){
               //省不通过的剔除
               $no_record = _model('convert_record')->read(array('apply_id' => $v['id'],'check_status' => 2));
               if($no_record){
                   unset($apply_info[$k]);
               }
           }
           $apply_info = $this->array_merge_rec($apply_info);
        }
        ...
        if($this->res_name == 'group'){
            foreach ($apply_list as $k => $v){
               //省不通过的
                $no_province = _model('convert_record')->read(array('apply_id' =>  $v['id'],'check_status' => 2));
                if($no_province){
                    unset($apply_list[$k]);
                }
            }
    ...
    public function load_apply()
    {
        ...
        $flag        = Request::Get('flag', 0);  //标识位是否是add页面进来
        $order = ' ORDER BY `create_time` DESC ';
        $filter = array('cid'=>$cid);
        $num_list = $plan_type = [];
        ...
        //add进入删除
        Response::assign('del_canerl', $flag);
     ...
     * @param unknown $array
     */
    public  function array_merge_rec($array) {
        // 定义一个新的数组
        $new_array = array ();
        // 遍历当前数组的所有元素
