...
class Action
{
    ...
    public function pdf_redirect()
    {
        ...
        Response::display('admin/file/pdf_show.html');
    ...
    public function record_list()
    {
        ...
        $page_no = Request::Get('page_no', 1);
        $search_filter = Request::Get('search_filter', array());// 当我不在你身边
        ...
        if (isset($search_filter['status']) && $search_filter['status']) {
            $memberids = file_apply_helper::get_record_member_ids($filter, $id, $search_filter['status']);
            if ($memberids) {
                $filter['id'] = $memberids;
            } else {
                $filter['id'] = '';
    ...
    public function export($list, $pc, $fileid)
    {
        ...
        foreach ($list as $k => $v) {
            ...
            // 是否操作过文件
            $file_info = file_apply_helper::check_member_record($v['id'], $fileid);
            $info[$k]['is_record'] = $file_info['res'];
            $info[$k]['last_record_time'] = $file_info['update_time'];
